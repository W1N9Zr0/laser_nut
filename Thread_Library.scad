		...
		)
{
	...
	function 	profileRatio(i)=		profileRatio*(1-backlash) / starts;

	function	threadAngle(i)=		threadAngle;

	function	threadPosition(i)=		( profileRatio(i) + threadHeightToPitch*(1+clearance)*tan( threadAngle(i) ) )/2; // vertical position only

	function 	threadHeight(i)=		pitch*threadHeightToPitch*(1+clearance); // outer diam of threads
	...
	function 	ShaftX(i)= 			0; 
	function 	ShaftY(i)= 			0;
	function 	ShaftZ(i)= 			pitch*numberTurns*i*starts; // this affects rot rate woo. also the thread hiehgt boot.
