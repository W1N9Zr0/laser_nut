...
module sweep(shape, path_transforms, closed=false, inverted=false) {

	function optionalreverse(list, reverse) = [for (i = [0:len(list)-1]) let(idx=reverse?len(list)-i-1:i) list[idx]];

    pathlen = len(path_transforms);
    segments = pathlen + (closed ? 0 : -1);
    shape3d = to_3d(shape);
	invertedShape3d = optionalreverse(shape3d, inverted);

    function sweep_points() =
      flatten([for (i=[0:pathlen-1]) transform(path_transforms[i], invertedShape3d)]);
    ...
    polyhedron(points = sweep_points(), faces = concat(loop_faces(), bottom_cap, top_cap), convexity=20);
